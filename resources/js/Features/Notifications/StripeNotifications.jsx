...
const StripeNotifications = ({ notifications, closePopUpNotification }) => {
  return (
    <div className='stripe-notifications'>
        <div id="toast-success" className={`${!notifications.status && `hidden` } flex items-center float-right p-4 mb-4 mr-4 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800`} role="alert">
            ...
            <div className="ml-3 text-sm font-normal">
                {notifications.type == 'success' ? `You have successfully subscribed to ${notifications.message} plan` : notifications.message}
                
            </div>
            <button type="button" onClick={() => closePopUpNotification({message: "", status: false, type: ""})} className="ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-success" aria-label="Close">
...
export default StripeNotifications